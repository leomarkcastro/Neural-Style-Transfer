...
def main():
    ...
    noise_level_img = 300                 # set AWGN noise level for noisy image
    ...
    for idx, img in enumerate(L_paths):
        ...
        for noise_level in [0,25,50,75,100,150,200,250,300]:

            noise_level_img = noise_level                 # set AWGN noise level for noisy image
            noise_level_model = noise_level_img  # set noise level for model
            ...
            logger.info('{:s} - PSNR: {:.2f} dB; SSIM: {:.4f}. Denoise Level: {}'.format(img_name+ext, psnr, ssim, noise_level))

            # ------------------------------------
            # save results
            # ------------------------------------

            util.imsave(img_E, os.path.join(E_path, img_name+f"denoised_{noise_level}"+ext))
